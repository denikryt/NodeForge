...
Static L-systems, where `ls_angle(...)` and `ls_step(...)` are compile-time numeric constants, use a baked generated-data backend. The compiler expands and interprets the turtle stream in Python, stores the result in a generated `Curve` datablock, creates a hidden generated `Object` for Object Info sourcing, and returns the Object Info `Geometry` output as a normal `Geometry` value. Branch-free runtime-angle or runtime-step L-systems use a generated command `Mesh` plus hidden generated `Object`; Object Info exposes that command table to a bounded field graph that reads command attributes, accumulates runtime heading/position, deletes non-draw edges, and converts the remaining mesh edges to curves. Branched runtime L-systems use a branch-aware command `Mesh` plus hidden generated `Object`; the mesh includes one synthetic anchor point per logical path, point-domain path/depth/parent-attach attributes, and edge-domain draw masks. The node graph computes path-local positions with grouped Accumulate Field, propagates branch origins through a depth-unrolled chain bounded by `MAX_LSYSTEM_BRANCH_DEPTH`, deletes non-draw edges, and converts the remaining mesh edges to curves. Exceeding the branch-depth budget raises controlled `CompileError`.
