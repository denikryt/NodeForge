...
def _parse_runtime_state_for(stmt):
    """Parse runtime Repeat Zone loops: for i in range(n): ..."""
    if not isinstance(stmt.target, ast.Name):
        raise CompileError("runtime range target must be a simple name, e.g. for i in range(n)")
    if not (_is_range_call(stmt, "range") and len(stmt.iter.args) == 1):
        raise CompileError("runtime range loop must look like: for i in range(n):")
    if not stmt.body:
        raise CompileError("runtime range loop body cannot be empty")

    def validate(sub):
        """Validate statements accepted inside runtime range loop bodies."""
        ...
        if isinstance(sub, ast.If):
            for branch_sub in list(sub.body) + list(sub.orelse):
                validate(branch_sub)
            return
        raise CompileError("runtime range body supports assignments and if blocks with assignments")
...
def _assigned_names_in_runtime_body(stmts):
    """Return assignment targets from a runtime range body in first-seen order."""
    names = []
...
def _repeat_state_assignments(group, comp, iterations, body_stmts, index_name=None, x=0, y=0):
    """Compile a range(...) loop with type-generic Repeat Zone state.
    ...
    if iterations.typ != TYPE_INT:
        raise CompileError("range(n) expects an Int value")
    ...
    if not state_names:
        raise CompileError("runtime range loop must update at least one existing variable")

    supported_state_types = {TYPE_GEOMETRY, TYPE_VECTOR, TYPE_FLOAT, TYPE_INT, TYPE_BOOL}
    for name in state_names:
        value = comp.vars[name]
        reject_compile_time_object(value, "runtime range state")
        if isinstance(value, list) or not isinstance(value, Value):
            raise CompileError("runtime range state must be a node value")
        if value.typ not in supported_state_types:
            raise CompileError(f"runtime range state {name!r} has unsupported type {value.typ}")

    if index_name in state_names:
        raise CompileError("runtime range loop index name cannot also be a state variable")
    ...
    if index_name in system_socket_names:
        raise CompileError(f"runtime range loop index name {index_name!r} conflicts with Repeat Zone socket name")
    for name in state_names:
        if name in system_socket_names:
            raise CompileError(f"runtime range state name {name!r} conflicts with Repeat Zone socket name")
    ...
    def compile_assign(sub):
        """Compile a runtime range assignment and update the current variable frame."""
        target = sub.targets[0].id
        val = comp.compile(sub.value)
        reject_compile_time_object(val, "runtime range assignment")
        if isinstance(val, list):
            raise CompileError("runtime range assignments cannot assign arrays")
        if target in state_set and not compatible_state_type(target, val):
            raise CompileError(f"runtime range state {target!r} changed type from {old_vars[target].typ} to {val.typ}")
    ...
    def compile_if(sub, depth=0):
        ...
        reject_compile_time_object(cond, "runtime range if condition")
        if cond.typ != TYPE_BOOL:
            raise CompileError("runtime range if condition must be Bool")
        ...
        for name in changed:
            ...
            reject_compile_time_object(true_val, "runtime range if branch merge")
            reject_compile_time_object(false_val, "runtime range if branch merge")
            if not isinstance(true_val, Value) or not isinstance(false_val, Value):
                raise CompileError("runtime range if can only merge node state values")
            if true_val.typ != false_val.typ:
                if false_val.typ == TYPE_INT and true_val.typ == TYPE_FLOAT:
                    ...
                    raise CompileError(f"runtime range if branch values for {name} have different types")
            ...
            if name in state_set and not compatible_state_type(name, merged):
                raise CompileError(f"runtime range state {name!r} changed type from {old_vars[name].typ} to {merged.typ}")
            comp.vars[name] = merged

    def compile_runtime_stmt(sub, depth=0):
        """Compile one statement accepted by a runtime range loop body."""
        if isinstance(sub, ast.Assign) and len(sub.targets) == 1 and isinstance(sub.targets[0], ast.Name):
            compile_assign(sub)
            return
        if isinstance(sub, ast.If):
    ...
    try:
        ...
        for name in state_names:
            ...
            if val is None:
                raise CompileError(f"runtime range state {name!r} was not assigned")
            reject_compile_time_object(val, "runtime range state output")
            if not compatible_state_type(name, val):
                raise CompileError(f"runtime range state {name!r} changed type from {old_vars[name].typ} to {val.typ}")
...
__all__ = [
    '_parse_runtime_for', '_parse_runtime_state_for', '_assigned_names_in_runtime_body',
